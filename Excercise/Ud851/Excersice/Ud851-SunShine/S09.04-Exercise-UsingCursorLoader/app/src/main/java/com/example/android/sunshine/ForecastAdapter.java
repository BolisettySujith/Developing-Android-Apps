...
//      TODO (7) Generate a weather summary with the date, description, high and low
        long dataInMills = mCursor.getLong(MainActivity.INDEX_WEATHER_DATE);

        String dataString = SunshineDateUtils.getFriendlyDateString(mContext,dataInMills,false);

        int weatherId = mCursor.getInt(MainActivity.INDEX_WEATHER_CONDITION_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext,weatherId);

        double highInCensus = mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);

        double lowInCEnsus = mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);

        String HighAndLowTemp = SunshineWeatherUtils.formatHighLows(mContext,highInCensus,lowInCEnsus);

        String weatherSummary = dataString+" - "+description+" - "+ HighAndLowTemp;
